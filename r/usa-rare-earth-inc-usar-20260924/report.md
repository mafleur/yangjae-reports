...
USAR는 **패스**입니다 — 엔진이 낸 유일한 셋업의 손익비가 1:0.43(손절 폭은 하루 평균 변동폭의 1.34배)으로, 직전 1.71에서 1 아래로 내려왔습니다. 진입 16.83 · 손절 15.36 · 목표 17.46달러인데 진입에서 목표까지가 0.63달러(하루 변동폭의 0.57배)뿐이라 걸 자리가 아닙니다. 16.83달러를 일봉 종가로 회복하면 그때는 매수가 아니라 재분석이고, 미진입 상태에서 이 계획을 접는 선은 14.99달러 일봉 종가 이탈이며(새 리포트 대기 / 파는 선 아님), 구조 자체의 폐기선은 13.87달러 이탈 후 3거래일 내 종가 회복 실패 + 반등에서 저항 확인입니다. 조건 점검은 9항목 중 2개 통과이고 미달은 20일선 위·이동평균 정배열·200일선 위·52주 고점 대비·지수 대비 20일 초과수익·상대강도선 신고가·손익비입니다. 장부의 30주는 15.83달러 전량 정리입니다.

> [!손절] 손절·무효화 기준
> **보유 30주**: 15.83달러에서 전량 정리입니다. 평단 17.8847달러 대비 -11.49%(-61.64달러)입니다. 직전 리포트가 16.78달러 정리를 지시했고 장부에 매도 기록이 아직 없어, 그 지시가 집행되지 않았다면 이번 가격에서 정리합니다.
> **신규 진입**: 하지 않습니다. 16.83달러에 진입했을 경우의 집행 손절은 15.36달러(진입 대비 -8.73%)이지만, 손익비가 1:0.43이라 이 계획으로는 진입 자체를 하지 않습니다.
> **미진입 상태에서 볼 선**: 14.99달러를 일봉 종가로 잃으면 이 리포트의 매수 계획은 끝납니다 — 새 리포트 대기이며 파는 선은 아닙니다. 현재가에서 -5.33%(하루 변동폭의 0.77배)입니다.
> **구조 폐기선**: 13.87달러를 이탈한 뒤 3거래일 내 종가로 회복하지 못하거나(또는 그 전에 12.77달러 아래로 종가 마감), 이후 반등에서 13.87달러가 저항으로 확인되면 박스 해석 자체를 접습니다. 현재가에서 하루 변동폭의 1.78배 거리로, 3배 안쪽이라 실무에서도 작동합니다.
...
|---|---|---|---|---|
| **회복선** (재분석 조건) | **16.83달러** | 일봉 종가 회복 — 손익비가 1 미만이므로 이 가격은 **사는 선이 아니라 다시 계산해 보는 값**입니다 | 회복 대기 | **+6.32%** |
...
![USAR 전략도 — 진입·손절·목표](/Users/ae-heesung/Claude/investor-in-yangjae/outputs/USAR_20260923.png)
...
① **관찰**: 일봉 종가가 16.83달러 아래로 마감 → 구조 이탈이 시작된 것이지만 되돌아 올라오면 속임수 하락일 수 있어 아직 무효가 아닙니다. 신규 진입만 중단하고 집행 손절은 그대로 둡니다.
② **경고**: 이탈 후 3거래일 안에 16.83달러를 종가로 회복하지 못함, **또는 그 전에 일봉 종가가 15.73달러 아래로 마감**(하루 평균 변동폭의 1배만큼 더 밀림) — 둘 중 먼저 오는 쪽. 잔여 비중을 줄이고 반등에서의 반응을 확인합니다.
③ **무효**: 반등이 16.83달러에서 막히고 그 아래로 되밀림(지지가 저항으로 전환) → 셋업 폐기.
...
세 갈래 중 지금 가장 두꺼운 것은 **횡보 지속**입니다. 저점은 절상 중이지만 지지 테스트 거래량이 혼조이고 상승봉 거래량 우위도 없어, 위로 풀릴 근거는 9월 21·22일 거래량(20일 평균의 1.35배)뿐입니다.
...
![USAR 구조도 — 근거 레벨](/Users/ae-heesung/Claude/investor-in-yangjae/outputs/USAR_20260923_structure.png)
...
![USAR 피보나치 되돌림 집중도](/Users/ae-heesung/Claude/investor-in-yangjae/outputs/USAR_20260923_fib.png)
...
![USAR 신호가 나온 날 — 날짜별 표시](/Users/ae-heesung/Claude/investor-in-yangjae/outputs/USAR_20260923_wyckoff.png)
...
**(b) 목표주가 분포.** 애널리스트 9명, 투자의견 strong_buy. 최저 21.00달러(+32.66%) / 평균 35.56달러(+124.61%) / 최고 45.00달러(+184.27%). 현재가 15.83달러는 **9명 전원의 목표 가운데 가장 낮은 21.00달러보다도 아래**입니다. 그 최저 목표는 이 셋업의 목표 17.46달러보다 3.54달러 위에 있습니다. 컨센서스는 12개월, 이 셋업은 수 주입니다.
...
- 이 엔진은 구조 손절 기준 손익비를 별도 항목으로 내지 않습니다. 이번 셋업은 집행 손절이 곧 구조 손절이라 두 값이 0.43으로 같습니다.
- 장부에는 2026-08-03 매수 30주(평단 17.8847달러)만 있고 매도 기록이 없습니다. 9월 11·15·17·22일 리포트가 낸 정리 지시가 집행됐는지는 확인이 필요하며, 체결을 지어 넣지 않았습니다.
